...
#include "MeasurementInfoList.h"   // ✅ CORRECTION AJOUTÉE
...
bool encode_kpm_indication(const std::string& kpi_name, double value1, int64_t value2, std::vector<unsigned char>& buffer)
{
    ...
    asn_long2INTEGER(&(mri2->choice.integer), (long)value2);  // ✅ CORRECTION AJOUTÉE
    ...
    // Assignation du format au message
    ind_msg->indicationMessage_formats.choice.indicationMessage_Format1 = format1;

    // --- Encodage ---
    void* encoded_buf = nullptr;
    ssize_t encoded_size = aper_encode_to_new_buffer(&asn_DEF_E2SM_KPM_IndicationMessage, nullptr, ind_msg, &encoded_buf);
    ...
    size_t encoded_len = (encoded_size + 7) / 8;
