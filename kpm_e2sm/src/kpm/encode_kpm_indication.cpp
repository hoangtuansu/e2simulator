...
bool encode_kpm_indication(const std::string& kpi_name, double value1, unsigned long value2, std::vector<unsigned char>& buffer)
{
    ...
    MeasurementData_t* meas_data_ptr = (MeasurementData_t*)calloc(1, sizeof(MeasurementData_t));
    if (!meas_data_ptr) {
        ASN_STRUCT_FREE(asn_DEF_E2SM_KPM_IndicationMessage, ind_msg);
        ASN_STRUCT_FREE(asn_DEF_MeasurementInfoList, format1->measInfoList);
        free(format1);
        return false;
    }
    format1->measData = *meas_data_ptr;
    free(meas_data_ptr);
    ...
    if (encoded_size <= 0 || !encoded_buf) {
        std::cerr << "\n[ERREUR] Echec encodage APER E2SM-KPM." << std::endl;
        ASN_STRUCT_FREE(asn_DEF_E2SM_KPM_IndicationMessage, ind_msg);
        return false;
    }
